...
class MantenimientoPreventivoForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        km_prox = cleaned_data.get('km_prox_mantenimiento')

        if vehiculo and km_prox is not None:
            if km_prox <= vehiculo.kilometraje_actual:
                self.add_error('km_prox_mantenimiento', 'Debe ser mayor que el kilometraje actual del vehículo.')
